...
class DeleteSysDictByIdDomainServiceTest {
    ...
    void deleteByIds_shouldSetDeleteStatusToTrue() {
        ...
        when(sysDictDomainRepository.load(new DictId(1L))).thenReturn(Mono.just(mockDomain));
        when(sysDictDomainRepository.save(any(SysDictDomain.class))).thenAnswer(invocation -> {
            SysDictDomain savedDomain = invocation.getArgument(0);
            return Mono.just(1L);
        });
